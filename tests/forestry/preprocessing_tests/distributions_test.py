...
class TestDistributions(test_util.ConverterTestSuite):
    ...
    def test_predict_sapling_diameters(self):
        ...
        reference_trees.vectorize({
            "identifier": [""] * 5,
            "height": [10.0, 10.0, 10.0, 10.0, 1.0],
            "breast_height_diameter": [8.0, 8.0, 8.0, 8.0, 8.0]
        })

        avghs = [20.0, 1.3, 1.2, 1.2, 999.0]
        avgds = [18.0, 0.0, 0.0, 10.0, 999.0]

        for i, (height, diameter) in enumerate(zip(avghs, avgds)):
            result = distributions.predict_sapling_diameters(reference_trees[i:i+1], height, diameter, 1.1)
            self.assertEqual(len(result), 1)
            self.assertEqual(result.height[0], assertions[i][0])
            self.assertEqual(result.breast_height_diameter[0], assertions[i][1])
