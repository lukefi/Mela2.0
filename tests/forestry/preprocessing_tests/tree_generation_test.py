...
class TestTreeGeneration(unittest.TestCase):
    ...
    def test_trees_from_weibull(self):
        ...
        n_trees = 10
        result = tree_generation._trees_from_weibull(fixture, n_trees=n_trees)
        self.assertEqual(10, len(result))
        self.assertEqual(14.67280367311326, result.breast_height_diameter[0])
        self.assertEqual(17.4, result.height[0])
        self.assertEqual(1.9357767362985978, result.stems_per_ha[0])
    ...
    def test_finalize_trees(self):
        ...
        self.assertEqual(1, result.species[1])
        self.assertEqual(25.0, result.breast_height_age[0])
        self.assertEqual(25.0, result.breast_height_age[1])
        self.assertEqual(32.0, result.biological_age[0])
        self.assertEqual(32.0, result.biological_age[1])
        self.assertEqual(1, result.tree_number[0])
        self.assertEqual(2, result.tree_number[1])
    ...
    def test_solve_tree_generation_strategy(self):
        strata = [
            ...
            TreeStratum_(mean_diameter=10.0, mean_height=8.0, stems_per_ha=55.0),
        ...
        for stratum, expected in zip(strata, expected_values):
            result = tree_generation._solve_tree_generation_strategy(stratum)
    ...
    def test_generate_reference_trees_from_tree_stratum(self):
        ...
        # Derive results
        results: list[ReferenceTrees] = []
        for test_stratum in test_data:
            result = tree_generation.reference_trees_from_tree_stratum(stand, test_stratum, n_trees=10)
        ...
        for (result, asse) in zip(results, expected_results):
            ...
            if n_reference_trees == 10:
                ...
                self.assertEqual(asse[2], result.species[0])
                self.assertEqual(asse[3], result.breast_height_diameter[0])
                self.assertEqual(asse[4], result.height[0])
                self.assertEqual(asse[5], result.stems_per_ha[0])
                self.assertEqual(asse[6], result.breast_height_age[0])
                self.assertEqual(asse[7], result.biological_age[0])
                self.assertEqual(asse[8], result.breast_height_diameter[1])
                self.assertEqual(asse[9], result.height[1])
                self.assertEqual(asse[10], result.stems_per_ha[1])
                self.assertEqual(asse[11], result.breast_height_age[1])
                self.assertEqual(asse[12], result.biological_age[1])
    ...
    def test_generate_sapling_reference_trees_from_tree_stratum(self):
        ...
        results = []
        for test_stratum in test_data:
            result = tree_generation.reference_trees_from_tree_stratum(stand, test_stratum, n_trees=n_trees)
            if result is not None:
                results.append(result)
        # Validate
        for (result, asse) in zip(results, expected_results):
