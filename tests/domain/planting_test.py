...
class PlantingTest(unittest.TestCase):
    def test_plant(self):
        ...
        regen = plnt._get_planting_instructions(
            stand.site_type_category,
            'tests/resources/planting_test/planting_instructions.txt')
        (stand, output) = plnt._plant(stand, collected_data, "regeneration", regen_species=TreeSpecies(
            regen['species']), rt_count=10, rt_stems=regen['stems/ha'], soil_preparation=regen['soil preparation'])
        self.assertEqual(200, stand.reference_trees.stems_per_ha[-1])
        ...
        self.assertEqual('1001-9-tree', stand.reference_trees.identifier[-1])
    ...
    def test_planting(self):
        ...
        self.assertEqual(TreeSpecies.PINE, output.prev("regeneration")['species'])
        self.assertEqual('1011-9-tree', stand.reference_trees.identifier[-1])
