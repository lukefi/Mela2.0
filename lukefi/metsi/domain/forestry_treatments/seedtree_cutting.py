...
def seedtree_cutting_fn(input_: ForestStand, /, **operation_parameters) -> OpTuple[ForestStand]:
    ...
    stem-count reduction when Motti is active. Then, if Motti state exists,
    marks all remaining YP trees as seed trees (puuluokka/tree class 3) and
    calls Motti4AfterSeedtreeCutting so Motti can create natural regeneration
    into the sapling vector when its conditions are satisfied.
    """
